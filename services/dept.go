...
func (self *DeptService) ReadOne(id uint) Model.Dept {
	...
	DB.First(&model, id).Related(&model.Users, "Users")
...
func (self *DeptService) Delete(id uint) error {
	...
	DB.Model(&model).Related(&model.Users, "Users")
	// tx.First(&model, id).Related(&model.Users, "Users")

	tx := DB.Begin()
	// delete users's user_proj
	for i := 0; i < len(model.Users); i++ {
		if err := tx.Model(&(model.Users[i])).Association("projs").Clear().Error; err != nil {
			tx.Rollback()
			return err
		}
	}
...
func (self *DeptService) Clear() error {
	...
	for i := 0; i < len(list); i++ {
		...
		DB.First(&model).Related(&model.Users, "Users")

		// delete users's user_proj
		for j := 0; j < len(model.Users); j++ {
			if err := tx.Model(&(model.Users[j])).Association("projs").Clear().Error; err != nil {
				tx.Rollback()
				return err
			}
		}
